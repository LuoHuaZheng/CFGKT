...
def run(args):
    ...
    raw_concept = train_df.ucid.unique().tolist()
    raw_question = train_df.upid.unique().tolist()
    sub_concept = {p: i for i, p in enumerate(raw_concept)}
    sub_question = {p: i for i, p in enumerate(raw_question)}
    train_df['ucid'] = train_df['ucid'].map(sub_concept)
    train_df['upid'] = train_df['upid'].map(sub_question)
    n_ques = len(train_df.upid.unique())
    n_cats = len(train_df.ucid.unique())
    print("no. of questions :", n_ques)
    print("no. of concepts: ", n_cats)
    ...
    train_dataset = CFGKT_dataset(train.values, n_ques=n_ques, n_concept=n_cats, max_seq=args.max_len)
    val_dataset = CFGKT_dataset(val.values, n_ques=n_ques, n_concept=n_cats, max_seq=args.max_len)
    test_dataset = CFGKT_dataset(test.values, n_ques=n_ques, n_concept=n_cats, max_seq=args.max_len)
    ...
    gc.collect()
    KT_model = CFGKT(n_cats=n_cats, n_pid=n_ques, d_model=args.embed_dim, n_blocks=args.n_blocks,
