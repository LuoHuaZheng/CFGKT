...
class KQN(nn.Module):
    ...
    def __init__(self, n_skills: int, n_hidden: int, n_rnn_hidden: int, n_mlp_hidden: int, dropout,
                 n_rnn_layers: int = 5, rnn_type='lstm', emb_type="qid", emb_path="", pretrain_dim=768):
        ...
        self.embed = nn.Embedding(self.num_c * 2 + 2, self.n_hidden, padding_idx=self.num_c*2+1)
        self.eye = torch.eye(n_skills)
        self.shft_embed = nn.Embedding(self.num_c+1,self.n_hidden, padding_idx=self.num_c)
    ...
    def forward(self, qa, qshft):
        in_data = self.embed(qa)
        next_skills = self.shft_embed(qshft)
