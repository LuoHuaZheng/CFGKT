...
class sparseKT(nn.Module):
    def __init__(self, n_question, n_pid,
        ...
        if self.n_pid > 0:
            if emb_type.find("scalar") != -1:
                ...
                self.difficult_param = nn.Embedding(self.n_pid + 2, 1).to(device)  # 题目难度
            else:
                self.difficult_param = nn.Embedding(self.n_pid + 2, embed_l).to(device)  # 题目难度
            self.q_embed_diff = nn.Embedding(self.n_question + 1,
                                             embed_l).to(device)  # question emb, 总结了包含当前question（concept）的problems（questions）的变化
            self.qa_embed_diff = nn.Embedding(2 * self.n_question + 2, embed_l).to(device)  # interaction emb, 同上
        ...
        if self.emb_type.endswith("predcurc"):  # predict cur question' cur concept
            ...
            self.qclasifier = Linear(self.hidden_size, self.num_c).to(device)
    ...
    def base_emb(self, q_data, target):
        q_embed_data = self.q_embed(q_data).to(device)  # BS, seqlen,  d_model# c_ct
    ...
    def get_attn_pad_mask(self, sm):
        ...
        pad_attn_mask = sm.data.eq(0).unsqueeze(1).to(device)
        pad_attn_mask = pad_attn_mask.expand(batch_size, l, l).to(device)
        return pad_attn_mask.repeat(self.nhead, 1, 1).to(device)
    ...
    # def forward(self, dcur, qtest=False, train=False, attn_grads=None, save_path="", save_attn_path="",
    #             save_grad_path="", attn_cnt_path=""):
    def forward(self, qid,cid,res, qtest=False, train=False, attn_grads=None, save_path="", save_attn_path="",
                save_grad_path="", attn_cnt_path=""):
        pid_data = qid
        q_data = cid
        target = res
        ...
        if emb_type in ["qid", "qidaktrasch", "qid_scalar", "qid_norasch"]:
            ...
            d_output, attn_weights = self.model(q_embed_data, qa_embed_data, emb_type, sparse_ratio, k_index,
                                                attn_grads, stride, save_path, save_attn_path, save_grad_path,
                                                attn_cnt_path, q_data, n_question)
            self.attn_weights = attn_weights
...
class Architecture(nn.Module):
    ...
    def forward(self, q_embed_data, qa_embed_data, emb_type="qid", sparse_ratio=0.8, k_index=8, attn_grads=None,
                stride=1, save_path="", save_attn_path="", save_grad_path="", attn_cnt_path="", q_data=None,
                n_question=None):
        ...
        for block in self.blocks_2:
            x, attn_weights = block(mask=0, query=x, key=x, values=y, apply_pos=True, emb_type=emb_type,
                                    sparse_ratio=sparse_ratio, k_index=k_index, attn_grads=attn_grads, stride=stride,
                                    save_path=save_path, save_attn_path=save_attn_path, save_grad_path=save_grad_path,
                                    attn_cnt_path=attn_cnt_path, q_data=q_data,
                                    n_question=n_question)  # True: +FFN+残差+laynorm 非第一层与0~t-1的的q的attention, 对应图中Knowledge Retriever
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, mask, query, key, values, apply_pos=True, emb_type="qid", sparse_ratio=0.8, k_index=8,
                attn_grads=None, stride=1, save_path="", save_attn_path="", save_grad_path="", attn_cnt_path="",
                q_data=None, n_question=None):
        """
        Input:
        ...
        if mask == 0:  # If 0, zero-padding is needed.
            ...
            query2, _ = self.masked_attn_head(
                query, key, values, mask=src_mask, zero_pad=True, emb_type=emb_type, sparse_ratio=sparse_ratio,
                k_index=k_index, attn_grads=attn_grads, stride=stride, save_path=save_path,
                save_attn_path=save_attn_path, save_grad_path=save_grad_path, attn_cnt_path=attn_cnt_path,
                q_data=q_data,
                n_question=n_question)  # 只能看到之前的信息，当前的信息也看不到，此时会把第一行score全置0，表示第一道题看不到历史的interaction信息，第一题attn之后，对应value全0
        else:
            # Calls block.masked_attn_head.forward() method
            query2, _ = self.masked_attn_head(
                query, key, values, mask=src_mask, zero_pad=False, emb_type=emb_type, sparse_ratio=sparse_ratio,
                k_index=k_index, attn_grads=attn_grads, stride=stride, save_path=save_path,
                save_attn_path=save_attn_path, save_grad_path=save_grad_path, attn_cnt_path=attn_cnt_path,
                q_data=q_data, n_question=n_question)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask, zero_pad, emb_type="qid", sparse_ratio=0.8, k_index=8, attn_grads=None, stride=1,
                save_path="", save_attn_path="", save_grad_path="", attn_cnt_path="", q_data=None, n_question=None):
        ...
        scores, attn_weights = attention(q, k, v, self.d_k,
                                         mask, self.dropout, zero_pad, emb_type, sparse_ratio=sparse_ratio,
                                         k_index=k_index, attn_grads=attn_grads, stride=stride, save_path=save_path,
                                         save_attn_path=save_attn_path, save_grad_path=save_grad_path,
                                         attn_cnt_path=attn_cnt_path, q_data=q_data, n_question=n_question)
...
def attention(q, k, v, d_k, mask, dropout, zero_pad, emb_type="qid", sparse_ratio=0.8, k_index=8, attn_grads=None,
              stride=1, save_path="", save_attn_path="", save_grad_path="", attn_cnt_path="", q_data=None,
              n_question=None):
    """
    ...
    scores = F.softmax(scores, dim=-1).to(device)  # BS,8,seqlen,seqlen
    ...
    #         sorted_scores, sorted_idx = torch.sort(scores_b, descending=True)
    #         scores_t = sorted_scores[:, k_index - 1:k_index].repeat(1, seqlen)
    #         scores_b = torch.where(scores_b - scores_t >= torch.tensor(0).to(device), scores_b,
    #                                torch.tensor(-1e32).to(device)).reshape(bs, head, seqlen - k_index, -1)
    #         scores = torch.cat([scores_a, scores_b], dim=2)
    #         scores = F.softmax(scores, dim=-1)  # BS,8,seqlen,seqlen
    #
    ...
    #     new_scores = torch.where(tmp_scores - scores >= 0, torch.tensor(-1e32).to(device).float(), scores).reshape(
    #         (bs, head, seqlen, -1))
